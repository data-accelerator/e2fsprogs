...
start_again:
	...
	if (err == EXT2_ET_NO_CURRENT_NODE) {
		...
		/*
		 * Only follow the hint while it still points inside the file
		 * system, so that the ~0ULL goal sentinel keeps working.
		 */
		if (fs->reserved[3] && fs->reserved[3] < max_blocks)
			goal = fs->reserved[3];
		if (goal == ~0ULL)
